...
class ModFusion(Module):
    ...
    def __init__(self, vadpy, options):
        ...
        if self.method == MAJORITY_METHOD:
            if not (self.inputs >= 3 and len(self.inputs) % 2 == 1):
                raise VADpyError('Labels attributes number is even or is less than 3')
    ...
    def run(self):
        ...
        if self.method == SIMPLE_HIST_METHOD:
            ...
            if not use_bayes:
                raise VADpyError('Simple histogram method was selected, but '
